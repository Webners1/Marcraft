  ...
  reducers: {
    ...
    filterFriendsByTag: (state, action) => {
      ...
      if (tag === 'All') {
        ...
        state.profiles = state.originalProfiles.filter((profile) => profile.tags.includes(tag));
      }
    },
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchFriends.fulfilled, (state, { payload }) => {
        state.status = 'succeeded';
        state.profiles = payload.payload.users;
      })
...
// Export the actions
export const { filterFriendsByCategory, filterFriendsByTag, filterFriendsByRating, resetFriends } =
